    ...
    while ing : 
        ...
        if not res_account : 
            ...
            while True :
                retry_count_int += 1
                try :
                    input_deposit_amount = int(input("* 출금 하실 금액을 입력해 주세요 : "))
                except ValueError :
                    ...
                    continue
                input_NY = input('* 출금하실 금액이 '+str(input_deposit_amount)+'원이 맞습니까?(Y/N) : ')
                if input_NY == 'N' :
                    continue
            ...
            try :
                if res_account[0][1] != int(input_accountpw) : 
                    ...
                    #거래기록 기재
                    sql_inset_trans = 'INSERT INTO TRANSACTION (trans_type, trans_amount, trans_balance, trans_account) VALUE (\'출금\',' + str(input_deposit_amount)+','+str(res_updated_account_balance[0][0])+',\''+input_accountid+'\')'
